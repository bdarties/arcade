export default class niveau3ciel extends Phaser.Scene {
  constructor() {
    super({ key: "niveau3ciel" });
  }

  

  preload() {
    this.load.image("fond_gauche_ciel", "./assets/sky1.jpg");
    this.load.image("fond_droit_ciel", "./assets/sky2.jpg");
    this.load.image("plateform_ciel", "./assets/tuyauciel.png");
    this.load.image("plateform2_ciel", "./assets/roueciel.png");
    this.load.image("plateform3_ciel", "./assets/chaineciel.png");
    this.load.image("plateformea_ciel", "./assets/champiciel.png");
    ...
    this.load.image("plateform4_ciel", "./assets/nuageciel.png");
  ...
  create() {
    ...
    this.fondGauche = this.add.image(320, 360, "fond_gauche_ciel").setScrollFactor(0);
    ...
    this.fondDroit = this.add.image(960, 360, "fond_droit_ciel").setScrollFactor(0);
    this.fondDroit.setOrigin(0.5, 0.5);
    this.fondDroit.setDepth(-1); // Placer en arrière-plan

    this.skinsPlateformes = ["plateform_ciel", "plateform2_ciel", "plateform3_ciel", "plateform4_ciel", "plateformea_ciel"];
    ...
    for (let x = 0; x <= 640; x += 64) {
      let pf = this.groupe_plateformes_J1.create(x - 32, 700, "plateform_ciel").setScale(0.5);
    ...
    for (let x = 640; x <= 1280; x += 64) {
      let pf = this.groupe_plateformes_J2.create(x - 32, 700, "plateform_ciel").setScale(0.5);
      pf.setOrigin(0.5, 0.5);
      pf.displayWidth = 64;
      pf.refreshBody();
    }
...
this.anims.create({
  ...
  frameRate: 1
    ...
    this.fondDroiteCam = this.add.image(320, 360, "fond_droit_ciel").setScrollFactor(0);
...
this.camera1.ignore(this.hudJ2); // visible seulement par cam 2
  ...
  genererInitiales(groupe, joueur) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (skin === "plateformea_ciel" && Math.random() < 0.7) {
            skin = Phaser.Utils.Array.GetRandom(this.skinsPlateformes.filter(s => s !== "plateformea_ciel"));
        }

        let pf;
        if (skin === "plateformea_ciel") {
...
genererProcedural(joueur) {
  ...
  updatePlateformesTimers(delta) {
    [...this.groupe_plateformes_J1.getChildren(), ...this.groupe_plateformes_J2.getChildren()].forEach(plat => {
      if (plat.texture.key === "plateformp_ciel" || plat.texture.key === "plateform4_ciel") {
