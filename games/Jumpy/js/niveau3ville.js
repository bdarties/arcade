export default class niveau3ville extends Phaser.Scene {
  constructor() {
    super({ key: "niveau3ville" });
  }

  

  preload() {
    this.load.image("fond_gauche_ville", "./assets/ville2.jpg");
    this.load.image("fond_droit_ville", "./assets/ville1.jpg");
    this.load.image("plateform_ville", "./assets/tuyauville.png");
    this.load.image("plateform2_ville", "./assets/roueville.png");
    this.load.image("plateform3_ville", "./assets/chaineville.png");
    this.load.image("plateformea_ville", "./assets/champiville.png");
    ...
    this.load.image("plateform4_ville", "./assets/tuyaucasse.png");
  ...
  create() {
    ...
    this.fondGauche = this.add.image(320, 360, "fond_gauche_ville").setScrollFactor(0);
    ...
    this.fondDroit = this.add.image(960, 360, "fond_droit_ville").setScrollFactor(0);
    this.fondDroit.setOrigin(0.5, 0.5);
    this.fondDroit.setDepth(-1); // Placer en arrière-plan

    this.skinsPlateformes = ["plateform_ville", "plateform2_ville", "plateform3_ville", "plateform4_ville", "plateformea_ville"];
    ...
    for (let x = 0; x <= 640; x += 64) {
      let pf = this.groupe_plateformes_J1.create(x - 32, 700, "plateform_ville").setScale(0.5);
    ...
    for (let x = 640; x <= 1280; x += 64) {
      let pf = this.groupe_plateformes_J2.create(x - 32, 700, "plateform_ville").setScale(0.5);
      pf.setOrigin(0.5, 0.5);
      pf.displayWidth = 64;
      pf.refreshBody();
    }
...
this.anims.create({
  ...
  frameRate: 1
    ...
    this.fondDroiteCam = this.add.image(320, 360, "fond_droit_ville").setScrollFactor(0);
...
this.camera1.ignore(this.hudJ2); // visible seulement par cam 2
  ...
  genererInitiales(groupe, joueur) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (skin === "plateformea_ville" && Math.random() < 0.7) {
            skin = Phaser.Utils.Array.GetRandom(this.skinsPlateformes.filter(s => s !== "plateformea_ville"));
        }

        let pf;
        if (skin === "plateformea_ville") {
...
genererProcedural(joueur) {
  ...
  updatePlateformesTimers(delta) {
    [...this.groupe_plateformes_J1.getChildren(), ...this.groupe_plateformes_J2.getChildren()].forEach(plat => {
      if (plat.texture.key === "plateformp_ville" || plat.texture.key === "plateform4_ville") {
  ...
  update(time, delta) {
    ...
    if (
      ...
      if (this.player1.body.touching.down) {
        ...
        this.sound.play("saut"); // ajuste ici la puissance du saut
