...
export default class SecondScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image("img_background5", "src/assets/map5.png");
    ...
    create() {
        ...
        // Ajouter le fond
        this.background = this.add.tileSprite(0, 0, 20000, 20000, "img_background5").setOrigin(0, 0);
        ...
        this.add.text(300, 50, 'Vous êtes dans la carte 5', { fontSize: '32px', fill: '#fff' }).setScrollFactor(0);
    }
    ...
    update(time, delta) {
        ...
        if (this.tempsEcoule >= 180 && !this.redRectangle) {
...
spawnEnemy() {
    ...
    if (distanceToPlayer > minDistance) {
        if (this.spawnMob2Allowed) {
            // Après 1m30, on fait apparaître soit mob1, mob2, mob3 ou le démon
            const random = Math.random();
            if (!this.demonSpawned) {
                // Faire apparaître le démon une seule fois après 1m30
                this.enemyManager.creerEnnemi(spawnX, spawnY, 'mob6');
                this.demonSpawned = true; // Marquer que le démon a été spawn
            } else if (random < 0.33) {
            ...
            // Avant 1m30, faire apparaître mob1
            this.enemyManager.creerEnnemi(spawnX, spawnY, 'mob1');
        }
        this.enemiesSpawned++;
    }
