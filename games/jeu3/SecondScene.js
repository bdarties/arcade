...
export default class SecondScene extends Phaser.Scene {
    ...
    preload() {
        // Charger les assets
        this.load.image("img_background2", "src/assets/mp2.png");
    ...
    create() {
        ...
        // Ajouter le fond
        this.background = this.add.tileSprite(0, 0, 20000, 20000, "img_background2").setOrigin(0, 0);
        ...
        this.time.addEvent({
            delay: 60000, // 1 minute en millisecondes
            callback: () => {
                this.spawnMob3Allowed = true;
                console.log("Les ennemis Mob3 peuvent maintenant apparaître.");
            },
            callbackScope: this,
            loop: false,
        });
        ...
        this.add.text(300, 50, 'Vous êtes dans la carte 2', { fontSize: '32px', fill: '#fff' }).setScrollFactor(0);
    }
...
spawnEnemy() {
    ...
    if (distanceToPlayer > minDistance) {
        if (this.spawnMob3Allowed) {
            // Après 1 minute, choisir aléatoirement entre mob1, mob2 et mob3
            ...
            // Pendant les premières 1 minute, seulement mob1 et mob2
            const random = Math.random();
            if (random < 0.5) {
                this.enemyManager.creerEnnemi(spawnX, spawnY, 'mob1');
            } else {
