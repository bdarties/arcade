...
export default class niveau2_speedrun extends Phaser.Scene {
  constructor() {
    super({ key: "niveau2_speedrun" }); 
  }
  ...
  create() {
    // Initialiser et jouer la musique
        ...
        musicManager.play('niveau2_speedrun');    // Configuration des touches pour le menu pause (P ou Y)
  ...
  this.clavier1.up = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.K);
    ...
    // Mise en place du suivi des joueurs avec lissage
    this.cameras.main.startFollow(this.player1);
    this.camera2.startFollow(this.player2, true, 0.12, 0.12);

    // Configuration des limites des caméras
  ...
  update() {
    ...
    try {
      if (this.timerRunning && this.domTimer && this.timerStart) {
        ...
        try { this.domTimer.innerText = this.formatTime(elapsed); } catch (e) {}
      }
    } catch (e) {}
        ...
        if (currentTime - enemy.lastShot >= enemy.shootCooldown) {
          ...
          enemy.once('animationcomplete', (animation) => {
            if (animation.key === 'enemy_tir_anim' && enemy.active && !enemy.isDead) {
              ...
              bullet.setVelocity(200 * direction, 30);
    ...
    if (this.porte_retour && this.doorActive) {
      ...
      if (p1Overlap && p1Press) {
        if (this.nbEngrenages >= this.totalEngrenages) {
          ...
          this.time.delayedCall(1000, () => {
            try {
              const elapsedMs = Math.max(0, (this.time.now || 0) - (this.timerStart || 0));
              if (this.domTimer && this.domTimer.style) this.domTimer.style.display = 'none';
              const name = window.prompt('Entrez votre nom pour le classement (laisser vide pour Anonyme):', '') || 'Anonyme';
              try { fct.saveSpeedrunScore('speedrun_scores_level2', name, elapsedMs); } catch (e) {}
              this.scene.start('victoire', { fromSpeedrun: true, elapsedMs });
            } catch (e) {
              this.scene.start('victoire', { fromSpeedrun: true });
            }
          });
        } else {
