      ...
      if ((p1Overlap && p2Overlap) && Phaser.Input.Keyboard.JustDown(this.keyR)) {
        ...
        this.time.delayedCall(1000, () => {
          try {
            ...
            try {
              const name = window.prompt('Entrez votre nom pour le classement (max 12 caractères)', 'Anonyme');
              try { fct.saveSpeedrunScore('speedrun_scores_level1', name ? name.substring(0,12) : 'Anonyme', finalTime); } catch (e) {}
            } catch (e) {
              try { fct.saveSpeedrunScore('speedrun_scores_level1', 'Anonyme', finalTime); } catch (ee) {}
            }
