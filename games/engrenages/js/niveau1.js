...
this.scene.stop('defaite');

    
    // Stocker le mode de jeu
    this.gameMode = data?.mode || 'histoire';

    // Réinitialiser les variables d'état du terminal
  ...
  this.keyR = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.R);
    ...
    if (terminalObj) {
      ...
      // S'assurer que le terminal est dans son état initial (sans teinte)
      this.porte_retour.clearTint();
      // Réinitialiser explicitement les flags d'activation
      this.porte_retour.player1HasActivated = false;
      this.porte_retour.player2HasActivated = false;
      // Activer explicitement le corps physique pour les collisions
      this.porte_retour.body.enable = true;
      // Définir une taille de hitbox appropriée
      this.porte_retour.body.setSize(64, 96);
      if (terminalObj.width && terminalObj.height) 
        this.porte_retour.setDisplaySize(terminalObj.width, terminalObj.height);

      // --- Sécurité : réactivation du corps physique du terminal ---
if (!this.porte_retour.body) {
  this.physics.world.enable(this.porte_retour);
  this.porte_retour.body.setAllowGravity(false);
  this.porte_retour.setImmovable(true);
}
this.porte_retour.body.enable = true;
