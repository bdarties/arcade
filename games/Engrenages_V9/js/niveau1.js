...
export default class niveau1 extends Phaser.Scene {
  ...
  create() {
    ...
    if (backgroundLayer3) backgroundLayer3.setDepth(0);
  ...
  // Note: on n'ajoute PAS de colliders globaux ici — les plateformes sont
  // traversables au départ. Les colliders seront créés/détruits par
  // activerBouton quand une plateforme devient solide.

    // --- FEU MONTANT ---
    // Le feu monte du bas vers le haut sur une durée configurable. Si
    // l'image 'feu' n'a pas été ajoutée dans assets/, on ignore proprement.
    if (this.textures && this.textures.exists && this.textures.exists('feu')) {
    // Position fixe en pixels : départ (1536,5920) -> cible (1536,1280)
    const feuX = 1536;
    const feuStartY = 6320;
    const feuTargetY = 1280;
      ...
      this.feu.setDepth(10);

      // Animation du feu : alterner rapidement entre les 4 états
      ...
      const riseDuration = 120000; // 15 secondes par défaut
      ...
      this.tweens.add({
        ...
        ease: 'Linear',
        onComplete: () => {
          if (this.feu && this.feu.destroy) this.feu.destroy();
        }
      });
  ...
  update() {
    // J1 (comportement copié depuis niveau2)
    ...
    if (this.clavier1.left.isDown) {
      ...
      if (isOnGround) {
        ...
        if (!this.player1.jumpPlayed) {
          this.player1.jumpPlayed = true;
          this.player1.anims.play("jump_left", false);
        }
      }
  ...
  // Méthode pour activer/désactiver un bouton et inverser la solidité des plateformes liées
  activerBouton(bouton, joueur) {
    // toggle de l'état du bouton
    bouton.isActivated = !bouton.isActivated;

    // conserve la couleur de base et applique une version plus claire à l'activation
    const baseColorMap = {
      jaune: 0xFFFF00,
      rouge: 0xFF0000,
      bleu: 0x6699FF,
    ...
    const base = baseColorMap[bouton.obstacleId];

    // helper: calcule une couleur plus claire (augmente chaque composante RGB de factor, cap 255)
    const brighten = (hex, factor = 1.4) => {
    ...
    const baseColor = base || null;
    const activeColor = baseColor ? brighten(baseColor, 1.35) : 0x99ff99;

    if (bouton.isActivated) {
      bouton.setTint(activeColor);
    } else {
      if (baseColor) bouton.setTint(baseColor);
      else bouton.clearTint();
    }

    // Met à jour toutes les plateformes liées au même obstacleId
    ...
    updateList.forEach(p => {
      ...
      // règle simple: portes inversent l'état par rapport au bouton, blocs suivent l'état
      p.estSolide = (p.role === 'porte') ? !bouton.isActivated : bouton.isActivated;

      if (p.estSolide) {
        p.setAlpha(1);
        if (p.body) p.body.enable = true;
        // créer un collider joueur <-> plateforme si pas déjà présent
        ...
        // retirer colliders s'ils existent
