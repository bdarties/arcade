export default class Player2 extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y) {
        ...
        // Suppression de la zone d'attaque automatique comme pour Player
    ...
    static createAnimations(scene) {
        const anims = [
            { key: "idle_right_p2", start: 0, end: 2, rate: 6 },
            { key: "idle_left_p2", start: 7, end: 9, rate: 6 },
            { key: "walk_right_p2", start: 14, end: 17, rate: 10 },
            { key: "walk_left_p2", start: 21, end: 24, rate: 10 },
            { key: "dash_right_p2", start: 28, end: 30, rate: 20 },
            { key: "dash_left_p2", start: 31, end: 33, rate: 20 }
        ];
        anims.forEach(a => {
    ...
    // Nouvelle méthode pour trouver l'ennemi le plus proche (identique à Player)
    findClosestEnemy() {
    ...
    // Méthode d'attaque manuelle (identique à Player)
    manualAttack() {
    ...
    // Méthode d'attaque (identique à Player)
    attack(target) {
        ...
        this.weapon.once("animationcomplete", () => {
            ...
            if (this.weapon.body) this.weapon.body.enable = false;
            try { 
                if (overlap) this.scene.physics.world.removeCollider(overlap); 
            } catch {}
        });
    }
    ...
    gainXP(amount) {
        this.xp += amount;
        this.scene.events.emit("xpChangedP2", this.xp, this.xpToNext);
    ...
    dash() {
        ...
        const animKey = (this.facing === "right") ? "dash_right_p2" : (this.facing === "left" ? "dash_left_p2" : null);
        ...
        const ghostTimer = this.scene.time.addEvent({
            ...
            callback: () => {
                const ghost = this.scene.add.sprite(this.x, this.y, this.texture.key, this.frame.name)
                    .setAlpha(0.6)
                    .setTint(0x88ff88) // Tint vert pour différencier du joueur 1
    ...
    update(controls) {
        ...
        if (!this.isDashing) {
            ...
            if (vx || vy) {
                ...
                const animKey = vx ? (this.facing === "right" ? "walk_right_p2" : "walk_left_p2") : (vy ? "walk_right_p2" : "");
                if (animKey) this.anims.play(animKey, true);
            } else {
                this.anims.play(this.facing === "right" || this.facing === "down" ? "idle_right_p2" : "idle_left_p2", true);
            }

            if (controls.dash?.isDown) this.dash();
            
            // Attaque manuelle (identique à Player)
    ...
    destroy(fromScene) {
        try { 
            this._zoneCollider && this.scene.physics.world.removeCollider(this._zoneCollider); 
        } catch {}
