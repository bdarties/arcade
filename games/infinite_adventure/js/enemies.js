...
class ProjectilePool {
  ...
  updateAll(player, player2, collisionLayer, propsCollisionLayer) {
  ...
  for (const proj of this.active) {
    ...
    if (player2 && p2x !== undefined && p2y !== undefined) {
      const dx2 = projX - p2x;
      const dy2 = projY - p2y;
      if (dx2 * dx2 + dy2 * dy2 < 144) {
        if (proj.enemyRef?.isAlive()) {
          proj.enemyRef.onProjectileHit(player2);
        }
        toRelease.push(proj);
        continue;
      }
...
export class Enemy {
  constructor(scene, x, y, config) {
    ...
    this.lastDamageByPlayer2 = false;
  ...
  die() {
    ...
    this.scene.addXP(this.config.xpValue, this.lastDamageByPlayer2);
...
export class Skull extends Enemy {
  ...
  if (!this.isAlive()) return;
  const playerIndex = player === this.scene.player2 ? 2 : 1;
  this.scene.damagePlayer(this.config.projectileDamage, playerIndex);
}
