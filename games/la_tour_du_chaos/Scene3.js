...
export default class SecondScene extends Phaser.Scene {
    ...
    preload() {
        // Charger les assets
        this.load.spritesheet("img_mob4", "src/assets/Mob4.png", {
            frameWidth: 64,
            frameHeight: 64,
        });
        this.load.image("img_background3", "src/assets/map3.png");
    ...
    create() {
        ...
        // Ajouter le fond
        this.background = this.add.tileSprite(0, 0, 20000, 20000, "img_background3").setOrigin(0, 0);
        ...
        this.time.addEvent({
            delay: 10000, // 1 minute en millisecondes
            callback: () => {
                this.spawnMob4Allowed = true;
                console.log("Les ennemis Mob4 peuvent maintenant apparaître.");
            },
            callbackScope: this,
            loop: false,
        });
        ...
        this.add.text(300, 50, 'Vous êtes dans la carte 3', { fontSize: '32px', fill: '#fff' }).setScrollFactor(0);
    }
...
spawnRandomEnemy() {
    ...
    if (distanceToPlayer > minDistance) {
        ...
        // Si le temps écoulé est supérieur à 1 minute, permettre le spawn de mob4
        if (this.spawnMob4Allowed) {
            this.enemyManager.creerEnnemi(spawnX, spawnY, 'mob4');
        }
