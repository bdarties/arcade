...
import PlayerConfig from './playconfig.js';
import PowerUpScene from './PowerUpScene.js'; // Importer la nouvelle scène // Importer PlayerConfig
...
export default class MainScene extends Phaser.Scene {
    constructor() {
        ...
        this.damageCoefficient = 1; // Coefficient pour les dégâts dans cette map
        this.healthCoefficient = 1; // Coefficient pour les points de vie dans cette map
    }

    preload() {
        // Charger les assets
        this.load.image('carte_orbe', 'src/assets/carte_degats_1.png');
        this.load.image('carte_degats', 'src/assets/carte_degats1.png');
        this.load.image('carte_soins', 'src/assets/carte_soins1.png');
        this.load.image('carte_cadence', 'src/assets/carte_candence1.png');
        this.load.image('carte_velocite', 'src/assets/carte_velocite1.png');
        this.load.image('carte_xp', 'src/assets/carte_xp1.png');
        this.load.spritesheet("img_demon", "src/assets/demon.png", {
            frameWidth: 128,
            frameHeight: 128,
        });
        ...
        this.load.spritesheet('orbe', 'src/assets/orbe.png', { frameWidth: 32, frameHeight: 32 });
        this.load.image("img_background", "src/assets/map1finale.png");
        this.load.image("img_etoile", "src/assets/diamant.png");
        this.load.image("bomb", "src/assets/boule1.png");
        this.load.image("boule", "src/assets/boule.png");
    ...
    create() {
        ...
        // Ajouter le fond
        this.background = this.add.tileSprite(0, 0, 20000, 20000, "img_background").setOrigin(0, 0);
        ...
        // Ajouter un texte indiquant la carte actuelle
        this.add.text(300, 50, 'Vous êtes dans la carte 1', { fontSize: '32px', fill: '#fff' }).setScrollFactor(0);
    }
    ...
    update(time, delta) {
        ...
        // Créer un rectangle rouge après 3 minutes
        if (this.tempsEcoule >= 150 && !this.redRectangle) {
    ...
    spawnEnemy() {
        ...
        if (distanceToPlayer > minDistance) {
            if (this.spawnMob2Allowed) {
                // Après 1m30, choisir aléatoirement entre mob1 et mob2
    ...
    levelUp() {
        ...
        // Mettre en pause MainScene et lancer PowerUpScene
        
        this.scene.launch('PowerUpScene', { player: this.player });  // Lancer PowerUpScene avec le joueur
this.scene.pause();  // Mettre en pause MainScene
    }
