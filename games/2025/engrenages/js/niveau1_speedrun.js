...
export default class niveau1_speedrun extends Phaser.Scene {
  constructor() {
    super({ key: "niveau1_speedrun" });
  }
...
//spritesheets ennemis 2
  ...
  create(data) {
    ...
    this.add.image(0, 0, 'ville_grotte')
      .setOrigin(0, 0) // Pour que l'image commence en haut à gauche
      .setDepth(-1); // Pour s'assurer que le background est derrière tous les autres éléments
      

    // Initialiser la musique du mode speedrun
    musicManager.scene = this;
    musicManager.play('niveau1_speedrun');
    ...
    */
    ...
    // --- Chronomètre HUD via overlay DOM (shared id speedrun-timer) ---
    try {
      ...
      if (hudContainer) {
        ...
        this.domTimer = span;
      } else this.domTimer = null;
      this.timerStart = this.time.now;
      this.timerRunning = true;
    } catch (e) { this.domTimer = null; this.timerStart = this.time.now; this.timerRunning = true; }
  ...
  // On définit les touches pour le terminal une seule fois à la création de la scène
  ...
  const hudSet = new Set([this.hudLifeJ1, this.hudLifeJ2]);
    ...
    //tir ennemi
    // Les projectiles des ennemis au sol ne sont pas affectés par la gravité
  ...
  update() {
    ...
    if (this.clavier1.up.isDown && isOnGround) {
      ...
      // Jouer un petit son de surprise/aspiration aléatoire (50% de chance)
      try {
  ...
  if (Math.abs(this.player2.smoothVel) < 2) this.player2.smoothVel = 0;
    ...
    if (this.porte_retour && !this.victoryTriggered) {
      ...
      if ((p1Overlap && p2Overlap) && Phaser.Input.Keyboard.JustDown(this.keyR)) {
        ...
        // Arrêter le chronomètre et sauvegarder le temps final
        this.timerRunning = false;
        const finalTime = Math.max(0, this.time.now - this.timerStart);
        if (this.domTimer) {
          this.domTimer.innerText = this.formatTime(finalTime);
          this.domTimer.style.color = '#00ff00'; // Met le chronomètre en vert pour indiquer la fin
        }
        
        this.time.delayedCall(1000, () => {
          try {
            if (this.domTimer && this.domTimer.style) this.domTimer.style.display = 'none';
            // ask name via native prompt and save score
            try {
              // Sauvegarder le score avec un nom automatique
              fct.saveSpeedrunScore('speedrun_scores_level1', '', finalTime);
            } catch (e) {
              console.error('Erreur lors de la sauvegarde du score:', e);
            }
            this.scene.start('victoire', { fromSpeedrun: true, elapsedMs: finalTime, mode: this.gameMode });
          } catch (e) {
            this.scene.start('victoire', { fromLevel: 2, mode: this.gameMode });
          }
        });
    ...
    if (this.timerRunning && this.domTimer) {
      const elapsedMs = Math.max(0, this.time.now - this.timerStart);
      this.domTimer.innerText = this.formatTime(elapsedMs);
    }
      ...
      if (distance <= enemy.detectionRange && currentTime - enemy.lastShot >= enemy.shootCooldown) {
        ...
        enemy.once('animationcomplete', (animation) => {
          if (animation.key === tirAnimKey && enemy.active && !enemy.isDead) {
            ...
            // Vitesse horizontale fixe avec une composante verticale vers le bas
            bullet.setVelocity(300 * direction, 30); // Composante verticale positive pour aller vers le bas
