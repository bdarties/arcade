...
function afficherTexteFlottant(scene, x, y, texte, options = {}) {
  const floatingText = scene.add.text(x, y, texte, {
    fontSize: options.fontSize || '20px', color: options.color || '#ffffff', fontFamily: options.fontFamily || 'Arial', stroke: options.stroke || '#000000', strokeThickness: options.strokeThickness || 4
  }).setOrigin(0.5);
...
export class LevelManager {
  ...
  showLevelUpEffect() {
    ...
    const levelUpText = this.scene.add.text(
      ...
      `LEVEL ${level}!`,
      {
        fontSize: '32px', color: '#ffff00', fontFamily: 'Arial', stroke: '#000000', strokeThickness: 6, fontStyle: 'bold'
      }
    ).setOrigin(0.5);
