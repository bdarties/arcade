...
export default class selection extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.spritesheet("explosion", "./assets/ennemi4/explosion.png", { frameWidth: 96, frameHeight: 96 });
    ...
    this.load.image("full_health_bar", "./assets/healthbar/full_health_bar.png");
 ...
 spawnVague(config) {
    ...
    for (let i = 0; i < config.totalEnnemis; i++) {
      this.time.delayedCall(i * 500, () => {
        ...
        if (i === config.totalEnnemis - 1) {
          ...
          console.log(`Tous les ennemis de la vague ${this.vagueActuelle} ont spawne`);
        }
      });
    }
  }
  ...
  verifierFinVague() {
    ...
    if (ennemisActifs === 0) {
      ...
      this.texteVague.setText(`VAGUE ${this.vagueActuelle} TERMINÉE !`).setColor("#44ff44");
      ...
      if (config.pauseApres > 0) {
        ...
        this.texteTimer.setText(`Vague ${prochaineVague} dans ${Math.ceil(this.timerPause)}s`).setColor("#44ff44");
      } else {
  ...
  finirToutesLesVagues() {
    ...
    this.texteTimer.setText("Toutes les vagues sont terminées !").setColor("#44ff44");
  ...
  update(time, delta) {
    ...
    if (this.enPause && this.timerPause > 0) {
      ...
      if (this.timerPause <= 0) {
        ...
        const prochaineVague = this.vagueActuelle + 1;
        this.texteTimer.setText(`Vague ${prochaineVague} dans ${tempsRestant}s`).setColor("#44ff44");
      }
    }
