...
export default class selection extends Phaser.Scene {
  ...
  create() {
    ...
    const ecart = 200;
    const centreX = 650;
    const portesX = [centreX - ecart, centreX, centreX + ecart];

    // Ajout des portes
    this.porte1 = this.physics.add.staticSprite(portesX[0], portesY, "img_porte1");
    this.porte2 = this.physics.add.staticSprite(portesX[1], portesY, "img_porte2");
    this.porte3 = this.physics.add.staticSprite(portesX[2], portesY, "img_porte3");

    // Textes au-dessus des portes
    this.add.text(portesX[0], portesY - 90, "Niveau 1", { fontSize: "24px", fill: "#fff" }).setOrigin(0.5);
    this.add.text(portesX[1], portesY - 90, "Niveau 2", { fontSize: "24px", fill: "#fff" }).setOrigin(0.5);
    this.add.text(portesX[2], portesY - 90, "Niveau 3", { fontSize: "24px", fill: "#fff" }).setOrigin(0.5);

    // Création du J1
  player = this.physics.add.sprite(portesX[1], portesY + 40, "img_perso");
  player.refreshBody();
